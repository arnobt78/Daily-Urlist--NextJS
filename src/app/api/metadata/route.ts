...
export async function GET(request: Request) {
  ...
  try {
    const response = await fetch(url, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; UrllistBot/1.0; +https://urlist.com)',
      },
    });
    ...
    const getTitle = (): string => {
      ...
      return ogTitle || twitterTitle || (titleMatch && titleMatch[1]) || 
             (h1Match && h1Match[1]) || new URL(url).hostname;
    };
